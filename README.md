# nStyle
next style
 #

 # Project Name

simple style for this feature in app 

## Features

- [x] Account Creation
- [x] Address Management
- [x] Ad-hoc Reporting
- [x] Admin Console
- [x] Advanced Search
- [x] API Integration
- [x] Bulk Uploading
- [x] Catalogue
- [x] Categories/Sub-categories
- [x] Connect Social Media Account
- [x] Contact Us
- [x] Coupon Code Generator
- [x] Customized Order Status
- [x] Dashboard
- [x] Discounts & Offers
- [x] Email List
- [x] Email Notifications
- [x] Facebook Login
- [x] Filter Items
- [x] Google Login
- [x] Interactive FAQs
- [x] Inventory Management
- [x] Invoice Billing
- [x] Live Chat
- [x] News Feed
- [x] Order Detail View
- [x] Order Management
- [x] Order Summary
- [x] Payment
- [x] Separate (Note: The correct spelling is "Separate.")
- [x] Feature
- [x] Word
- [x] Auto Blur Faces in the Images
- [x] Camera Access
- [x] Captcha
- [x] Certification Tracking
- [x] CF: Object Identification 2
- [x] CF: Object Tagging
- [x] CF: Video Object Identification
- [x] Content Approval
- [x] Content Flag
- [x] Content Management
- [x] Content Moderation
- [x] Conversation Threading
- [x] Custom Form
- [x] Dashboard Guests
- [x] Data Encryption
- [x] Deep Linking
- [x] Elastic Search
- [x] Flipkart Integration for Vouchers
- [x] Form Approval Workflow
- [x] Free Credits
- [x] Gamification
- [x] Geofence
- [x] Geo-location Reporting
- [x] Google Fit API
- [x] Google Vision
- [x] Hashtags
- [x] Image Training in Checklist
- [x] In-app Notifications
- [x] Information Masking
- [x] Invite Friends
- [x] Karma Points
- [x] Landing Page
- [x] Language 4
- [x] Language 5
- [x] Language Detection
- [x] Language Options
- [x] Language Support
- [x] Language Switch
- [x] Leaderboard
- [x] Like a Post
- [x] Location-based Alerts
- [x] Login Time Out
- [x] Machine Learning (ML) Image Categorization
- [x] Markup Tools
- [x] Match Algorithm
- [x] Mentions/Tagging
- [x] Microsoft Defender Integration
- [x] Myntra Integration for Vouchers
- [x] Notification Settings
- [x] Notifications over MS Teams
- [x] Object Identification in Video
- [x] Object Identifier
- [x] OCR (Optical Character Recognition)
- [x] Office 365 Login
- [x] People Management
- [x] Performance Tracker
- [x] Phone Login
- [x] Phone Verification
- [x] Photo Library
- [x] Polling
- [x] Post Creation
- [x] Project/Task Tracking
- [x] Push Notifications
- [x] QR Code Scanner
- [x] Question Bank
- [x] Repeating Tasks
- [x] Roles and Permissions
- [x] Scoring
- [x] Service Specific Settings (Admin)
- [x] Session Reporting
- [x] Share
- [x] Signup/Login
- [x] Single Sign-On (SSO)
- [x] Sorting
- [x] Speech to Text Conversions
- [x] Splash Screen
- [x] Story
- [x] Subtitles
- [x] Surveys
- [x] Tablet Design 1
- [x] Tablet Design 2
- [x] Tablet Design 3
- [x] Tablet Design 4
- [x] Task Allocator
- [x] Task Groups
- [x] Task List
- [x] Text Identification

- [x] Buy Now, Pay Later with Stripe
- [x] Calendar
- [x] Chat
- [x] Collect Transaction Fees
- [x] Content Management
- [x] Custom Form
- [x] Document Storage
- [x] Google Calendar Sync
- [x] Landing Page
- [x] Maps
- [x] Navigation Menu
- [x] Order Creation
- [x] Pricing Engine
- [x] Reservations
- [x] Reviews
- [x] Roles and Permissions
- [x] Settings
- [x] Shopping Cart
- [x] Signup/Login
- [x] Sorting
- [x] Store Locator
- [x] Terms and Conditions
- [x] User Profile
- [x] Product Description
- [x] Zip-Codes Search

## Usage

Instructions on how to use or set up the project go here.

## Contributing

Guidelines for contributing to the project go here.

## License

This project is licensed under the [License Name] - see the [LICENSE.md](LICENSE.md) file for details.
